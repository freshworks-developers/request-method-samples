...
async function getStarWarsCharDetails() {
  ...
  try {
    let starwarsCharDetail = await client.request.invokeTemplate(
...
async function searchForStarship() {
  ...
  const starshipManufracturerElement = document.querySelector(
    "#starshipManufacturer"
  );
  const starshipClassElement = document.querySelector("#starshipClass");
  try {
    let starshipDetail = await client.request.invokeTemplate(
      "starwarsStarshipSearch",
      { context: { starshipname: "Death Star" } }
    );
    ...
    starshipManufracturerElement.innerHTML +=
      starshipDetailJSON.results[0].manufacturer;
    starshipClassElement.innerHTML +=
      starshipDetailJSON.results[0].starship_class;
  } catch (err) {
